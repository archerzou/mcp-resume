#!/usr/bin/env python3
"""
Script to create or update a GitHub gist with Archer's resume data.
...
def create_or_update_gist(github_token: str, gist_id: str = None):
    ...
    resume_file = Path("data/archer_zou_resume.json")
    if not resume_file.exists():
        print("❌ Resume file not found: data/archer_zou_resume.json")
        return None
    ...
    gist_data = {
        "description": "Archer Zou - Resume (JSON Resume Format)",
        "public": True,
        "files": {
            "resume.json": {
                "content": resume_data
